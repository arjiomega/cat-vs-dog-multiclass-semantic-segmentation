...
class DataSplitter:
    ...
    def __init__(self,input_list:list[str],mask_label:dict[str,int],img_ext:str,mask_ext:str,args:dict):
    ...
    def train_valid_test_split(self,img_list,mask_list,shuffle):
        ...
        assert round(self.train_split + self.valid_split + self.test_split,1) == 1.0, "sum of train,valid,test must be equal to 1"

        train_count = int(self.train_split*(len(img_list)))
        valid_count = int((self.train_split+self.valid_split)*(len(img_list)))
        test_count =  int((self.train_split+self.valid_split+self.test_split)*(len(img_list)))

...
class DiceLoss(tf.keras.losses.Loss):
    def __init__(self, name="DiceLoss", smooth=1e-7, gamma=1.0, **kwargs):
    ...
    def call(self, y_true, y_pred):
        ...
        numerator = (
            2 * tf.reduce_sum(tf.multiply(y_pred, y_true), axis=(1, 2)) + self.smooth
        )
        denominator = (
            tf.reduce_sum(y_pred**self.gamma, axis=(1, 2))
            + tf.reduce_sum(y_true**self.gamma, axis=(1, 2))
            + self.smooth
        )

        # get the average dice loss of all the classes
        # shape (batch_size,)
        dice_loss_per_batch = 1 - tf.reduce_mean(
            tf.divide(numerator, denominator), axis=-1
        )
...
class TverskyLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        numerator = (
            tf.reduce_sum(tf.multiply(y_pred, y_true), axis=(1, 2), keepdims=True)
            + self.smooth
        )
        ...
        denominator = (
            tf.reduce_sum(tf.multiply(y_pred, y_true), axis=(1, 2), keepdims=True)
            + tf.reduce_sum(
                self.beta * tf.multiply((1 - y_true), y_pred),
                axis=(1, 2),
                keepdims=True,
            )
            + tf.reduce_sum(
                (1 - self.beta) * tf.multiply(y_true, (1 - y_pred)),
                axis=(1, 2),
                keepdims=True,
            )
            + self.smooth
...
class FocalTverskyLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        # shape (1,1,n_classes)
        numerator = (
            tf.reduce_sum(tf.multiply(y_pred, y_true), axis=(0, 1), keepdims=True)
            + self.smooth
        )

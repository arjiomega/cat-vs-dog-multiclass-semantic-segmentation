...
def VGG16_Unet_decoder(expansive_input, contractive_input, n_filters=32):
    # deconvolution
    # new shape = [(shape + 2(padding) - kernel_size)/stride]  + 1 (reverse this formula for deconvolution new shape)
    # doubles shape size
    up = tfl.Conv2DTranspose(
        filters=n_filters, kernel_size=3, strides=2, padding="same"
    )(expansive_input)
...
def VGG16_Unet(n_classes):
    ...
    ## fifth block
    conv_5 = VGG16_Unet_decoder(
        expansive_input=conv_last2, contractive_input=contract_blocks[4], n_filters=512
    )

    ## fourth block
    conv_4 = VGG16_Unet_decoder(
        expansive_input=conv_5, contractive_input=contract_blocks[3], n_filters=512
    )

    ## third block

    conv_3 = VGG16_Unet_decoder(
        expansive_input=conv_4, contractive_input=contract_blocks[2], n_filters=256
    )

    ## second block
    conv_2 = VGG16_Unet_decoder(
        expansive_input=conv_3, contractive_input=contract_blocks[1], n_filters=128
    )

    ## first block

    conv_1 = VGG16_Unet_decoder(
        expansive_input=conv_2, contractive_input=contract_blocks[0], n_filters=64
    )
    ...
    conv_out = tfl.Conv2D(
        filters=n_classes, kernel_size=1, padding="same", activation="softmax"
    )(conv_out)
